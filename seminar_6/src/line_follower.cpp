...
LineFollower::LineFollower() : Node("LineFollowingNode") {
    ...
    this->declare_parameter<int>("kP", 1);                 // 1–10
                                     // Коэффициент пропорциональной составляющей ПИД-регулятора.
                                     // Слишком большой (>10) → робот "болтается" из стороны в сторону.
                                     // Слишком маленький (<1) → вялая реакция, съезд с линии на поворотах.
    
    this->declare_parameter<int>("kI", 6);                 // 1–10
                                     // Коэффициент интегральной составляющей ПИД-регулятора.
                                     // Слишком большой (>10) → накопление ошибки, перерегулирование.
                                     // Слишком маленький (<1) → медленное возвращение на линию после отклонения.
    
    this->declare_parameter<int>("lower_threshold", 50);   // 50–60
                                     // Нижний порог фильтра Кэнни (Canny edge detection).
                                     // Слишком низкий (<50) → много шума, ложные срабатывания.
                                     // Слишком высокий (>60) → пропуск краёв линии, особенно на поворотах.
    
    this->declare_parameter<int>("upper_threshold", 100);  // 90–100
                                     // Верхний порог фильтра Кэнни.
                                     // Слишком низкий (<90) → обрывы линии на изгибах.
                                     // Слишком высокий (>100) → захват посторонних объектов и теней.
    // ===========================================================
